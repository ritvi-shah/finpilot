...
if st.sidebar.button("▶ Load 3-month demo",use_container_width=True):
    st.session_state.df=demo_transactions()
    st.session_state.uploaded_name="demo"
    st.rerun()
...
if df.empty:
    ...
    <p>Upload a transaction statement from the sidebar. You can also load the built-in 3-month demo for the hackathon presentation.</p>
    <div class="insight"><b>Privacy-first:</b> the prototype does not need bank credentials or a live banking connection.</div>
    </div>""",unsafe_allow_html=True)
    st.markdown("### Minimum import format")
    st.code("Date, Description, Amount")
    st.markdown("Optional: `Type, Category, Notes`")
    st.stop()
